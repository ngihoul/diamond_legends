    ...
    <div className='player-profile-container'>
      ...
      <div className='data'>
        <div className='section generic-data'>
          <ul>
            <li>
              <span className='label'>Date de naissance : </span> {moment(player.dateOfBirth).format('DD/MM/YYYY')} -{' '}
              {age(player.dateOfBirth)} ans
            </li>
            <li>
              <span className='label'>Lance : </span> {HandedType[player.throw]}
            </li>
            <li>
              <span className='label'>Frappe : </span> {HandedType[player.bat]}
            </li>
            <li>
              <span className='label'>Salaire : </span> ${player.salary.toFixed(2)}
            </li>
          </ul>
        </div>
        <div className='section in-game'>
          <ul>
            <li>
              ...
              <Link className='link' href={`/game/team/${player.team.id}`}>
                {player.team.name} ({player.team.abbreviation.toUpperCase()})
              </Link>
            </li>
            <li>
              <span className='label'>Position{player.positions.length > 1 ? 's' : ''} :</span>{' '}
              {player.positions.map((pos) => PositionType[pos]).join(', ')}
            </li>
            <li>
              <span className='label'>Energie : </span> {player.energy} / 100
            </li>
          </ul>
        </div>
      </div>
